...
Csnippet("
	...
	") -> rmeas
Csnippet("
         Nobs = rnorm(N,pow(sigma_obs,2)*pow(pow(sigma_obs2,2),(time+600)/100));
         ") -> rmeas
...
Csnippet("
         lik = dnorm(Nobs,N,pow(sigma_obs,2)*pow(pow(sigma_obs2,2),(time+600)/100),give_log);
         ") -> dmeas
Pomps = list()
...
lower = c(a = 0,sigma=1,N_0 = 0,sigma_obs=0.2,z = .8,d = -0.6,b = -0.5,c =-.5,e=-0.3,f=-0.3,sigma_obs2=1)
upper = c(a = 0.6,sigma=1.4, N_0 = .3,sigma_obs=.4,z = 1,d = 1,b = 0.5,c = .5,e=-0.3,f=-0.3,sigma_obs2=.9)
